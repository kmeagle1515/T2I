...
sys.path.insert(0, '/scratch/mnk2978/DL/MINIPROJ2/GAN-INT-CLS/T2I/models')
...
import os
from resolution_test import res_tes

class Trainer(object):
    def __init__(self, type, dataset, split, lr, diter, vis_screen, save_path, l1_coef, l2_coef, pre_trained_gen, pre_trained_disc, batch_size, num_workers, epochs, superesolution):
...
#                     fake_images_cpu = fake_images.data.cpu().numpy()[:64] * 128 + 128
#                     print(fake_images_cpu)
#                     print(fake_images_cpu.shape)
...
#                    self.logger.draw(right_images, fake_images)
                ...
                fake_images_cpu = fake_images.data.cpu().numpy()[:64]
                weights_file = 'srcnn_x3.pth'
                scale = 3
                for image in fake_images_cpu:
                    img = np.moveaxis(image, 0, -1)
                    img = Image.fromarray((img*255).astype(np.uint8))
                    img = img.convert('RGB')  #check if rgb
                    res_tes(weights_file, img, supercnt, scale)
                    supercnt+=1
                ...
                for image in fake_images_cpu:
                    ...
                    img = Image.fromarray((img*255).astype(np.uint8))
                    img = img.convert('RGB')  #check if rgb
                    img.save("fake_images/fakeimg{}.png".format(fakecnt))
                    fakecnt+=1
                
                
                real_images_cpu = right_images.data.cpu().numpy()[:64]
                
                for image in real_images_cpu:
                    img = np.moveaxis(image, 0, -1)
                    img = Image.fromarray((img*255).astype(np.uint8))
                    img = img.convert('RGB')  #check if rgb
                    img.save("real_images/realimg{}.png".format(realcnt))
                    realcnt +=1
...
#         self.logger.globplot_epoch_w_scores()
